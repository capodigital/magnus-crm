...
- The Meta template creation payload now explicitly declares `parameter_format: positional` for the starter flow and preserves `error_data.details` when Meta returns an API error, so Vercel users receive the relevant validation detail instead of only `Invalid parameter`.
- Production diagnosis note: the `confirmacion_cita` input received by Vercel is structurally valid for a positional `UTILITY` template. The current workspace cannot inspect Vercel's deployment or production token; if the error persists, first verify the production token has template-management access and that the tenant's bound WABA is the one that owns the production phone, using Settings sync as the read-access check.
...
## Next safe action
In the Vercel deployment, use Settings > Sincronizar con Meta. If synchronization fails, correct the production token/WABA permissions or binding first; if it succeeds, retry **Usar confirmacion cita** and capture the Vercel function log for `POST /api/whatsapp/templates` if Meta still rejects it. Share only the endpoint, HTTP status, Meta code/type/subcode/details, template name, and language, never the token. After approval, verify an out-of-window approved-template send and its status webhook. Implement per-tenant encrypted tokens through Embedded Signup before opening onboarding to other tenants.
