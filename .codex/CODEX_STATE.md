...
## Next safe action
Deploy this validated build to Vercel, confirm its production `DATABASE_URL`, `META_ACCESS_TOKEN`, and WABA/phone binding are the intended ones, then use **Usar confirmacion cita** from Settings. Fill the two review examples and submit; if Meta still rejects it, capture the new error detail from the Vercel function response without sharing the token. After approval, verify an out-of-window approved-template send and its status webhook. Implement per-tenant encrypted tokens through Embedded Signup before opening onboarding to other tenants.
